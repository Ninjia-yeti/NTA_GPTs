...
def get_pca():
    n_components = min(REDUCED_DIMENSION, min(1000, EMBEDDING_DIMENSION))  # Ensure n_components is valid
    return PCA(n_components=n_components, svd_solver='randomized')  # Use randomized solver


def reduce_embedding_dimension(embedding, pca):
...
def process_text_files(folder_path):
    ...
    embedding_metadata = []  # Collect metadata (content, file_name, chunk_index)
    ...
    for file_name in os.listdir(folder_path):
        ...
        for chunk_index, chunk in enumerate(chunks):
            try:
                ...
                # Collect embeddings and metadata for batch processing
                all_embeddings.append(embedding)
                embedding_metadata.append((chunk, file_name, chunk_index))

                print(f"Processed chunk {chunk_index} of {file_name}.")
            except Exception as e:
                print(f"Error processing chunk {chunk_index} of {file_name}: {e}")

    # Apply PCA if there are enough embeddings
    if len(all_embeddings) > 1:
        print("Applying PCA for dimensionality reduction...")
        pca = get_pca()
        reduced_embeddings = pca.fit_transform(np.array(all_embeddings))

        # Insert reduced embeddings into the database
        for i, (chunk, file_name, chunk_index) in enumerate(embedding_metadata):
            insert_into_db(chunk, file_name, chunk_index, reduced_embeddings[i])
            print(f"Inserted chunk {chunk_index} of {file_name} into the database.")
    else:
        print("Not enough embeddings for PCA. Skipping dimensionality reduction.")

    print("Embedding generation process is complete.")
